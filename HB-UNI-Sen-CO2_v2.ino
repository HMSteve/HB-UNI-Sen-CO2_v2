...
//select CO2 and optional pressure sensor 
//#define useSCD30
#define useSCD41
#define useBME280 
...
// Pin definitions Stephan's HB-UNI-Sen-CO2 Board v2.0 plus soft-on correction for CC1101_PWR_SW_PIN
...
#define CC1101_PWR_SW_PIN   11 //pcb version 2.0 plus soft-on correction
//#define CC1101_PWR_SW_PIN   27 //pcb version 1.0
...
class WeatherChannel : public Channel<Hal, SensorList1, EmptyList, List4, PEERS_PER_CHANNEL, SensorList0>, public Alarm {
  ...
  public:
    ...
    virtual void trigger (__attribute__ ((unused)) AlarmClock& clock) {
      ...
      if (waitSCD41)
      {
        DPRINTLN("start SCD41 single shot measurement and come back in 5 sec"); 
        tick = seconds2ticks(5);
        clock.add(*this);
        if (pressureAmb>0) {
          scd41.measure(pressureAmb);            
        } else {
          scd41.measure();
        }        
        waitSCD41 = false;
      ...
      if (!firstMsg) {
      ...
      #elif defined useSCD41       
        co2 = scd41.carbondioxide();
        temp10 = scd41.temperature() - this->device().getList0().tempOffset10();
        ...
        DDEC(device().battery().current());DPRINT(" / ");
        DDECLN(co2);
    ...
    void setup(Device<Hal, SensorList0>* dev, uint8_t number, uint16_t addr) {
      ...
      #elif defined useSCD41
        scd41.init(device().getList0().altitude(), this->device().getList0().tempOffset10(), false);      
      #endif
      #if defined useBME280
        bme280.init();
      #endif      
    ...
    bool forceCalib()
    {
      ...
      #elif defined useSCD41
        return(scd41.setForcedRecalibrationFactor(this->getList1().co2CalibRef()));     
      #endif
    }
...
void setup () {
  ...
  delay(10); // wait for soft-on to complete
  //init ePaper display
